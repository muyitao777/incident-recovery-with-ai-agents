import { ExternalLink, Clock, ChevronRight, Share2, Search, Activity, ArrowRight, XCircle, CheckCircle, RotateCcw } from 'lucide-react';
...
export function QuickRemediation() {
  ...
  const handleShareClick = () => {
    ...
    const content = `AI recommends rollback to v2.3.9 to restore service health. Failed deployment: ${failedDeployment?.version} (${failedDeployment?.description}) deployed by ${failedDeployment?.author} at ${failedDeployment?.time}. Recent successful deployments available: v2.3.9, v2.3.8.`;
    setShareContent({
  ...
  const handleSeeAgentActivity = () => {
    // Highlight agent a3 (Deployment Correlation) for deployment-related remediation
        ...
        <div className="flex items-start gap-2 mb-2.5">
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Rollback to <span className="font-bold text-cyan-700">v2.3.9</span> to restore service health
            </p>
          </div>
        </div>
        
        <div className="flex gap-1.5">
          <button className="relative overflow-hidden bg-gradient-to-r from-cyan-500 via-blue-500 to-blue-600 hover:from-cyan-400 hover:via-blue-400 hover:to-blue-500 text-white px-3 py-1.5 rounded-lg text-xs transition-all duration-300 font-semibold hover:scale-105 flex items-center gap-1.5">
          ...
          <button className="bg-white hover:bg-slate-50 text-slate-700 px-2.5 py-1.5 rounded-lg text-xs transition-all duration-200 border border-slate-200 font-semibold hover:border-slate-400">
            View Diff
          </button>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {deployments.map((deployment, idx) => (
            <div key={idx} className={`group relative flex items-center gap-2.5 p-3 rounded-lg border transition-all ${
              ...
              <button 
                className="relative overflow-hidden bg-gradient-to-r from-cyan-500 via-blue-500 to-blue-600 hover:from-cyan-400 hover:via-blue-400 hover:to-blue-500 text-white px-2.5 py-1.5 rounded-lg text-xs transition-all duration-300 font-semibold hover:scale-105 flex items-center gap-1.5 flex-shrink-0"
