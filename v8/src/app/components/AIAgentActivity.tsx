import { CheckCircle2, Loader2, Clock, ChevronDown, ChevronRight, FileText, SlidersHorizontal } from 'lucide-react';
...
export function AIAgentActivity() {
  ...
  return (
    <div className="bg-white rounded-xl pt-4 pr-4 pb-4 pl-5 border border-slate-200 shadow-[0_1px_8px_0_rgba(0,0,0,0.03)]">
      {/* Header */}
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-gradient-to-r from-cyan-50 to-blue-100 rounded-full border border-cyan-200">
            <div className="w-1.5 h-1.5 bg-cyan-500 rounded-full animate-pulse"></div>
            <span className="text-xs font-medium text-cyan-700 whitespace-nowrap">Active {inProgressCount}</span>
          </div>
        </div>
      </div>
        ...
        {filteredActions.map((action, index) => (
          <div key={action.id}>
            <button
              ...
              className={`w-full flex items-start gap-2.5 p-3 transition-all text-left bg-white hover:bg-slate-50 border border-slate-200 ${expandedAction === action.id ? 'rounded-t-lg border-b-0' : 'rounded-lg'}`}
            >
              {/* Content */}
              <div className="flex-1 min-w-0">
                <div className="mb-1">
                  <h4 className="text-xs font-semibold text-slate-900">{action.action}</h4>
                  {action.duration && action.status === 'completed' && (
                    <span className="text-xs text-slate-400">{action.duration}</span>
                  )}
                </div>
                <p className="text-xs text-slate-600 leading-relaxed mb-1.5">{action.details}</p>
                {action.confidence && action.status === 'in-progress' && (
                  <div className="flex items-center gap-1.5 mt-1.5">
                    ...
                    <span className="text-xs font-medium text-slate-600">{action.confidence}%</span>
                  </div>
                )}
              </div>
