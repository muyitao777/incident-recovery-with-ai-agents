...
import { Activity, AlertTriangle, AlertCircle, CheckCircle, Folder, Wrench, Flame, Plus, RefreshCw, ChevronRight, Search, ListFilter } from 'lucide-react';
...
export function InvestigateTabContent({
  ...
  const [selectedFilters, setSelectedFilters] = useState<string[]>(['healthy', 'warning', 'critical', 'unknown']);
  ...
  const statusFilters = [
    { id: 'healthy', label: 'Healthy', color: 'bg-green-500' },
    { id: 'warning', label: 'Warning', color: 'bg-amber-500' },
    { id: 'critical', label: 'Critical', color: 'bg-red-500' },
    { id: 'unknown', label: 'Unknown', color: 'bg-slate-400' }
  ];
  ...
  return (
    <div className="w-full space-y-3">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-slate-50 rounded-lg p-3">
          <div className="flex items-center justify-between mb-3">
            <div className="text-sm font-medium text-slate-900">Service Dependency Graph</div>
          ...
          <div className="mb-3 flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
              ...
              <div className="flex items-center gap-2">
                {statusFilters.map((filter) => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full transition-all border ${
      ...
      <div className="grid grid-cols-2 gap-3">
        {/* Left Column */}
        <div className="bg-white rounded-lg border border-slate-200 shadow-[0_1px_8px_0_rgba(0,0,0,0.03)]">
          <DetailedMetrics />
        </div>
      </div>
    </div>
